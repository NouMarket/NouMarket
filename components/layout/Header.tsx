...
export default function Header() {
  ...
  function categoryLabel(slug: string) {
    return t(`category.${slug}` as TranslationKey);
  }
  ...
  const languageSelect = (
    <select
      ...
      className="h-9 rounded-lg border border-gray-200 bg-white px-2 text-xs font-semibold text-gray-600 outline-none transition-colors hover:bg-gray-50 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20"
    >
      {locales.map((item) => (
    ...
    <header className="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-sm border-b border-gray-100 shadow-sm">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-2">
            <div className="relative">
              ...
              >
                {t("common.categories")} <ChevronDown className="h-4 w-4" />
              </button>
              {categoryOpen && (
                <div className="absolute top-full right-0 mt-1 w-56 bg-white border border-gray-100 rounded-2xl shadow-xl p-2 z-50">
                  {CATEGORIES.map((cat) => (
                    ...
                    >
                      ...
                      <span>{categoryLabel(cat.slug)}</span>
                    </Link>
                  ))}
                </div>
              )}
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 py-4 space-y-1">
          ...
          <label className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-700">
            <span className="font-medium">{t("language.label")}</span>
            <select
              value={locale}
              onChange={(event) => setLocale(event.target.value as Locale)}
              aria-label={t("language.label")}
              className="ml-auto rounded-lg border border-gray-200 bg-white px-2 py-1 text-xs font-semibold"
            >
              {locales.map((item) => (
                <option key={item} value={item}>
                  {item === "fr" ? t("language.fr") : t("language.tr")}
                </option>
              ))}
            </select>
          </label>
          ...
          <div className="pt-2 border-t border-gray-100 space-y-1">
            ...
            {user ? (
              ...
              <Link
                ...
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-700 hover:bg-gray-50"
              >
                <User className="h-4 w-4" /> {t("nav.loginRegister")}
              </Link>
            )}
          </div>
        </div>
